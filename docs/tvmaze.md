...
This document tracks the implementation status of all TVmaze API endpoints in `crates/animedb/src/provider.rs`.
...
| Show episodes | GET | `/shows/{id}/episodes` | `page`, `specials` | ❌ Not Implemented |
...
- **Embedding:** TVmaze supports embedding related resources via `?embed=cast,seasons,episodes` query params on show endpoints.
